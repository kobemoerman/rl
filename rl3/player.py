...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        annealing = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon_current = annealing.value(current_total_steps)

        eps = np.random.uniform(0,1)
        if eps < 1-epsilon_current:
            action = np.nanargmax(Q[state])
        else:
            action = np.random.choice(all_actions)
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        return self.initial_p + de*(t/self.schedule_timesteps)
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
